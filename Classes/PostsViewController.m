...
- (void)refreshPostList {
  ...
  /*  
	...
	requestFetch.entity = [NSEntityDescription entityForName:@"Post" inManagedObjectContext:appDelegate.managedObjectContext];
    NSError *error2 = nil;
	catArray = [appDelegate.managedObjectContext executeFetchRequest:requestFetch error:&error2];
	[requestFetch release];
    
    for (Post *newPost in catArray) {
        NSLog(@"%s newPost = %@",__FUNCTION__,newPost.postTitle);
    }
    */
}
