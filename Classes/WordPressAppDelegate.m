...
#import "TabloidSplitViewController.h"
...
//            NSLog(@"tempIsAuthentia = %@", tempIsAuthenticated);
...
//	 NSLog(@"%s",__FUNCTION__);
    ...
    TabloidFrontVC *tabloidFrontVC = [[TabloidFrontVC alloc] initWithNibName:@"TabloidFrontVC" bundle:nil];
    UINavigationController *aNavigationController = [[UINavigationController alloc] initWithRootViewController:tabloidFrontVC];
    aNavigationController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
    aNavigationController.modalPresentationStyle = UIModalPresentationFullScreen;
  //  self.navigationController = aNavigationController;
  //  [mainViewController presentModalViewController:aNavigationController animated:YES];
  //  [aNavigationController release];
    [tabloidFrontVC release];
...
//		[window addSubview:mainViewController.view];
//		[window makeKeyAndVisible];
//        NSLog(@"DeviceIsIPAD with blogcount = %d", [Blog countWithContext:context]);
//		if ([Blog countWithContext:context] == 0)
//		{
//            NSLog(@"Blog count is 0");
...
//            
//            TabloidWelcomePageVC *tabloidWelcomeViewController = [[TabloidWelcomePageVC alloc] initWithNibName:@"TabloidWelcomePageVC" bundle:nil];
//            UINavigationController *aNavigationController = [[UINavigationController alloc] initWithRootViewController:tabloidWelcomeViewController];
//            aNavigationController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
//			aNavigationController.modalPresentationStyle = UIModalPresentationFormSheet;
//			self.navigationController = aNavigationController;
//			[mainViewController presentModalViewController:aNavigationController animated:YES];
//			[aNavigationController release];
//			[tabloidWelcomeViewController release];
//            
//		}
//        
////        TabloidCountryViewController *countryViewController = [[TabloidCountryViewController alloc] initWithNibName:@"TabloidCountryViewController" bundle:nil];
////        countryViewController.view.frame = CGRectMake(0, 0, 384, 502);
////        [mainViewController.view addSubview:countryViewController.view];
////        
//
...
- (void)showContentDetailViewController:(UIViewController *)viewController {
    NSLog(@"%s here1",__FUNCTION__);
	if (self.splitViewController) {
         NSLog(@"%s here2",__FUNCTION__);
        ...
        if (viewController) {
         ...
         NSLog(@"%s here5",__FUNCTION__);
		[self.navigationController pushViewController:viewController animated:YES];
	}
    else if (self.mainViewController)
    {
        NSLog(@"mainViewLoaded..do sth");
    }
     NSLog(@"%s here6",__FUNCTION__);
}
...
- (NSString *)applicationUserAgent {
      ...
      NSDictionary *dictionary = [[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"wp-iphone/Tabloid"], @"UserAgent", nil];
   // return [[NSUserDefaults standardUserDefaults] objectForKey:@"UserAgent"];
    return @"wp-iphone/Tabloid";
}
...
- (void)splitViewController: (UISplitViewController*)svc willHideViewController:(UIViewController *)aViewController withBarButtonItem:(UIBarButtonItem*)barButtonItem forPopoverController: (UIPopoverController*)pc {
	...
	[barButtonItem setTitle:NSLocalizedString(@"Post Title", @"")];
...
//	}
	if ([[self.detailNavigationController.viewControllers objectAtIndex:0] isKindOfClass:[TabloidSplitViewController class]])
	{
		[[CPopoverManager instance] setCurrentPopoverController:pc];
	}
...
- (void)showPopoverIfNecessary {
	if (UIInterfaceOrientationIsPortrait(self.masterNavigationController.interfaceOrientation) && !self.mainViewController.modalViewController) {
		UINavigationItem *theNavigationItem = [[self.detailNavigationController.viewControllers objectAtIndex:0] navigationItem];
	...
	if (alertView.tag == 101) { //Password Alert
		...
		switch(buttonIndex) {
			case 0: {
				...
				if (DeviceIsPad() && self.mainViewController.modalViewController) {
					...
					if (DeviceIsPad()) {
						...
						[mainViewController presentModalViewController:helpViewController animated:YES];
					}
					else
